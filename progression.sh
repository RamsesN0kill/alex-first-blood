#!/bin/bash
# ============================================================
# 🩸 Alex First Blood — Suivi de progression
# ============================================================
# Tableau de bord de ton avancement dans le programme de prépa
# (Bachelor 1 Cybersécurité). Ta progression est sauvegardée en
# local dans .progression.state (fichier perso, non versionné).
#
# UTILISATION
...
#   ./progression.sh done 1 badge    -> valide la room/plateforme en ligne
...
# Titres des badges (index 1..10)
TITRES=( "" \
  "Survivant du Terminal" \
  "Premier Sang Python" \
  "Forgeron des Données" \
  "Dompteur des Pointeurs" \
  "Architecte du Front" \
  "Bâtisseur du Back" \
  "Maître des Bases" \
  "Cartographe des Réseaux" \
  "Maître des Machines" \
  "Alex First Blood — Opérationnel" )

THEMES=( "" \
  "Linux, terminal & Git" \
  "Python — algo & bases" \
  "Python — structures & projet" \
  "Le langage C (pointeurs)" \
  "Web front (HTML/CSS/JS)" \
  "Web back (PHP 8)" \
  "Bases de données (SQL/UML)" \
  "Réseaux (OSI, IP, CCNA 1)" \
  "Systèmes & virtualisation" \
  "Synthèse & projet final" )
...
valide_arg() {
  if ! [[ "$1" =~ ^([1-9]|10)$ ]]; then echo "❌ Numéro de semaine invalide (1 à 10)."; exit 1; fi
...
dashboard() {
  local total_etapes=30 faits=0 badges=0
  echo
  echo "  🩸 ALEX FIRST BLOOD — TABLEAU DE BORD D'ALEXANDRE"
  echo "  ══════════════════════════════════════════════════════════"
  echo
  ...
  printf "  Badges gagnés: %d/10\n" "$badges"
  echo
  if [ "$badges" -eq 10 ]; then
    echo "  🎖️  FÉLICITATIONS ALEXANDRE — tu es opérationnel. Premier sang versé : tu es prêt pour le Bachelor !"
  else
