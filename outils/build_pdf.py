...
# -*- coding: utf-8 -*-
"""
Générateur de PDF pour « Alex First Blood — Manuel d'apprentissage ».
...
# Palette « First Blood » (rouge/crimson) — distincte du manuel CyberHero (bleu).
# Les noms BLEU/BLEU_CLR sont conservés pour ne pas toucher le moteur de rendu :
# seules leurs valeurs RVB changent.
BLEU      = (140, 20, 30)      # titres (crimson profond)
BLEU_CLR  = (190, 45, 55)      # accents (rouge vif)
GRIS_CODE = (245, 244, 244)    # fond des blocs de code
GRIS_TXT  = (60, 60, 60)
ENCADRE   = (250, 234, 235)    # fond des encadrés "à retenir" (rose pâle)
ENCADRE_B = (190, 45, 55)      # barre des encadrés
NOIR      = (20, 20, 20)
BLANC     = (255, 255, 255)
    ...
    for ri, row in enumerate(cells):
        ...
        for ci, w in enumerate(wrapped):
            ...
            if header:
                ...
                pdf.set_fill_color(250, 246, 246) if ri % 2 else pdf.set_fill_color(*BLANC)
                pdf.set_text_color(*NOIR)
...
def cover(pdf):
    ...
    centre("Alex First Blood", 34, "B", top=70)
    pdf.ln(4)
    centre("Manuel d'apprentissage", 18)
    pdf.ln(2)
    centre("Les fondamentaux de l'informatique, en mots simples", 13, "I")
    pdf.set_draw_color(*BLANC); pdf.set_line_width(0.5)
    pdf.line(60, 150, pdf.w - 60, 150)
    centre("Préparé pour Alexandre", 13, top=160)
    centre("Avant l'entrée en Bachelor 1 Cybersécurité", 11)
    centre("10 chapitres pour partir de zéro et arriver prêt.", 10, "I", top=pdf.h - 30)
